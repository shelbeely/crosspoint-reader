...
void GhostActivity::updateMacString() {
  snprintf(macStr, sizeof(macStr), "%02X:%02X:%02X:%02X:%02X:%02X", currentMac[0], currentMac[1], currentMac[2],
      ...
      if (mappedInput.wasReleased(MappedInputManager::Button::Confirm)) {
        switch (settingIndex) {
          ...
          case 1:
            ...
            if (config.macRotateIntervalMs <= 30000)
              config.macRotateIntervalMs = 60000;
            else if (config.macRotateIntervalMs <= 60000)
              config.macRotateIntervalMs = 120000;
            else if (config.macRotateIntervalMs <= 120000)
              config.macRotateIntervalMs = 300000;
            else
              config.macRotateIntervalMs = 30000;
            break;
          case 2:
            // Cycle TX power: 8 (2dBm stealth) -> 44 (11dBm medium) -> 84 (20dBm max) -> 8
            if (config.txPower <= 8)
              config.txPower = 44;
            else if (config.txPower <= 44)
              config.txPower = 84;
            else
              config.txPower = 8;
            break;
          ...
          case 5:
            ...
            if (config.deadManMinutes <= 5)
              config.deadManMinutes = 15;
            else if (config.deadManMinutes <= 15)
              config.deadManMinutes = 30;
            else if (config.deadManMinutes <= 30)
              config.deadManMinutes = 60;
            else
              config.deadManMinutes = 5;
            break;
        }
        saveConfig();
        requestUpdate();
...
void GhostActivity::renderDashboard() const {
  ...
  // MAC rotation countdown
  {
    ...
    if (config.macRotateEnabled && macRotationActive) {
      ...
      unsigned long remaining =
          (elapsed < config.macRotateIntervalMs) ? (config.macRotateIntervalMs - elapsed) / 1000 : 0;
  ...
  // Probe suppression
  {
    renderer.drawText(UI_10_FONT_ID, leftPad, y, config.probeSuppress ? "Probes: SUPPRESSED" : "Probes: NORMAL", true);
    y += lineH;
  }
  ...
  // Radio state
  {
    ...
    switch (RADIO.getState()) {
      case RadioManager::RadioState::WIFI:
        radioLabel = "Radio: WIFI";
        break;
      case RadioManager::RadioState::BLE:
        radioLabel = "Radio: BLE";
        break;
      default:
        radioLabel = "Radio: OFF";
        break;
    }
  ...
  for (int i = 0; i < DASH_ACTION_COUNT; i++) {
    ...
    if (selected) {
      renderer.fillRect(leftPad - 4, y - 2, pageWidth - (leftPad - 4) * 2, renderer.getLineHeight(UI_10_FONT_ID) + 4,
                        true);
    }
    char row[32];
...
void GhostActivity::renderSettings() const {
  ...
  // 0: MAC Rotation
  snprintf(labels[0], sizeof(labels[0]), "MAC Rotation:     %s", config.macRotateEnabled ? "ON" : "OFF");

  // 1: Rotate Interval
  {
    uint32_t s = config.macRotateIntervalMs / 1000;
    snprintf(labels[1], sizeof(labels[1]), "Rotate Interval:  %lus", static_cast<unsigned long>(s));
  }
  ...
  // 3: Probe Suppress
  snprintf(labels[3], sizeof(labels[3]), "Probe Suppress:   %s", config.probeSuppress ? "ON" : "OFF");

  // 4: Dead Man Switch
  snprintf(labels[4], sizeof(labels[4]), "Dead Man Switch:  %s", config.deadManEnabled ? "ON" : "OFF");

  // 5: Dead Man Timer
  snprintf(labels[5], sizeof(labels[5]), "Dead Man Timer:   %u min", static_cast<unsigned>(config.deadManMinutes));

  // Use drawList with prebuilt strings
  const int contentTop = metrics.topPadding + metrics.headerHeight + metrics.verticalSpacing;
  const int listH = renderer.getScreenHeight() - contentTop - metrics.buttonHintsHeight - metrics.verticalSpacing;

  GUI.drawList(renderer, Rect{0, contentTop, pageWidth, listH}, SETTING_COUNT, settingIndex,
