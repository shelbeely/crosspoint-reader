...
void ScanActivity::loop() {
  ...
  if (state == BROWSE_CATEGORIES) {
    ...
    if (mappedInput.wasReleased(MappedInputManager::Button::Confirm)) {
      browseType = (categoryIndex == 0) ? TargetType::AP : (categoryIndex == 1) ? TargetType::STA : TargetType::BLE;
      refreshBrowseList();
      state = BROWSE_LIST;
      requestUpdate();
      return;
  ...
  if (state == TARGET_DETAIL) {
    ...
    if (mappedInput.wasReleased(MappedInputManager::Button::PageForward)) {
      if (detailIndex >= 0 && detailIndex < browseCount && browseList[detailIndex]) {
        char path[64];
        snprintf(path, sizeof(path), "/biscuit/profiles/%02x%02x%02x%02x%02x%02x.json", browseList[detailIndex]->mac[0],
                 browseList[detailIndex]->mac[1], browseList[detailIndex]->mac[2], browseList[detailIndex]->mac[3],
...
void ScanActivity::renderIdle() const {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, "SCAN");
...
void ScanActivity::renderDashboard() const {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, "SCAN — ACTIVE", headerSub);
...
void ScanActivity::renderCategories() const {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, "SCAN — BROWSE");
  ...
  GUI.drawList(
      renderer, Rect{0, listTop, pageWidth, listHeight}, 3, categoryIndex,
...
void ScanActivity::renderBrowse() const {
  ...
  if (browseCount == 0) {
    renderer.drawCenteredText(UI_10_FONT_ID, listTop + listHeight / 2 - renderer.getTextHeight(UI_10_FONT_ID) / 2,
                              "No targets found.");
  } else {
    GUI.drawList(
        renderer, Rect{0, listTop, pageWidth, listHeight}, browseCount, browseIndex,
        [this](int i) -> std::string {
          if (i < 0 || i >= browseCount || !browseList[i]) return "";
          const Target* t = browseList[i];
          char mac[20];
          snprintf(mac, sizeof(mac), "%02X:%02X:%02X:%02X:%02X:%02X", t->mac[0], t->mac[1], t->mac[2], t->mac[3],
                   t->mac[4], t->mac[5]);
        ...
        [this](int i) -> std::string {
          ...
          const Target* t = browseList[i];
          char sub[48];
          const char* typeStr = (t->type == TargetType::AP) ? "AP" : (t->type == TargetType::STA) ? "Client" : "BLE";
...
void ScanActivity::renderTargetDetail() const {
  ...
  if (detailIndex < 0 || detailIndex >= browseCount || !browseList[detailIndex]) {
    GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, "SCAN — DETAIL");
    renderer.displayBuffer();
    return;
  }
  ...
  char mac[20];
  snprintf(mac, sizeof(mac), "%02X:%02X:%02X:%02X:%02X:%02X", t->mac[0], t->mac[1], t->mac[2], t->mac[3], t->mac[4],
           t->mac[5]);

  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, "DETAIL", mac);
  ...
  if (t->type == TargetType::AP) {
    ...
    snprintf(buf, sizeof(buf), "Auth:    %s   PMF: %s   WPS: %s", authName, t->pmf ? "Yes" : "No",
             t->wps ? "Yes" : "No");
    renderer.drawText(UI_10_FONT_ID, leftPad, y, buf);
    y += lineGap;

    snprintf(buf, sizeof(buf), "Clients: %d   Handshake: %s   PMKID: %s", (int)t->clientCount,
             t->hasHandshake ? "Yes" : "No", t->hasPmkid ? "Yes" : "No");
    renderer.drawText(UI_10_FONT_ID, leftPad, y, buf);
    y += lineGap;
  }

  // First / last seen
  snprintf(buf, sizeof(buf), "First:   %lus ago",
           (unsigned long)(esp_timer_get_time() / 1000000ULL) - (unsigned long)t->firstSeen);
  renderer.drawText(SMALL_FONT_ID, leftPad, y, buf);
  y += smallH + 4;

  snprintf(buf, sizeof(buf), "Last:    %lus ago",
           (unsigned long)(esp_timer_get_time() / 1000000ULL) - (unsigned long)t->lastSeen);
  renderer.drawText(SMALL_FONT_ID, leftPad, y, buf);
  y += smallH + 4;
