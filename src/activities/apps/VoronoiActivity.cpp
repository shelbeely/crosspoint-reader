...
static void fillDithered75(GfxRenderer& r, int x, int y, int w, int h) {
  ...
  for (int dy = 0; dy < h; dy += 2)
    for (int dx = ((dy / 2) % 2); dx < w; dx += 2) r.drawPixel(x + dx, y + dy, false);
}
...
void VoronoiActivity::render(RenderLock&&) {
    ...
    for (int gx = 0; gx < gridW; gx++) {
      ...
      switch (pattern) {
        case 0:
          break;  // white (empty)
        case 1:
          fillDithered25(renderer, px, py, GRID_STEP, GRID_STEP);
          break;
        case 2:
          fillDithered50(renderer, px, py, GRID_STEP, GRID_STEP);
          break;
        case 3:
          fillDithered75(renderer, px, py, GRID_STEP, GRID_STEP);
          break;
      }
    }
  }
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, tr(STR_VORONOI), info.c_str());
