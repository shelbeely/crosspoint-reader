...
void HuntActivity::analyzeCapabilities() {
  ...
  if (current->type == TargetType::AP) {
    add("Handshake capture", current->pmf ? "PMF enabled — may fail" : "No PMF — feasible", !current->pmf, CAP_FIRE, 3);
    add("PMKID harvest", "Passive — no client needed", current->authType >= 3, CAP_FIRE, 4);
    add("Portal clone", "Clone AP + serve portal page", true, CAP_FIRE, 8);
    add("Beacon flood", "Flood nearby device WiFi lists", true, CAP_FIRE, 5);
    if (current->wps) {
      add("WPS assessment", "No automated tool available", false, CAP_NONE, -1);
    }
  } else if (current->type == TargetType::STA) {
    add("Probe response", current->probeCount > 0 ? "Has probed SSIDs — feasible" : "No probes seen",
        current->probeCount > 0, CAP_FIRE, 7);
    add("Track movement", "Monitor RSSI over time", true, CAP_TRACK, -1);
    add("AP association spoof", "Respond as known AP to client", current->probeCount > 0, CAP_FIRE, 7);
  } else if (current->type == TargetType::BLE) {
    add("Clone advertisement", "Copy BLE broadcast data", true, CAP_FIRE, 9);
    add("Enumerate services", "Connect and list GATT services", true, CAP_FIRE, 11);
    add("Track proximity", "Monitor RSSI distance", true, CAP_TRACK, -1);
  }
}
...
void HuntActivity::onExit() { Activity::onExit(); }
  ...
  switch (state) {
    case SELECT_CATEGORY: {
      ...
      if (mappedInput.wasReleased(MappedInputManager::Button::Confirm)) {
        if (huntCategoryIndex == 0)
          browseType = TargetType::AP;
        else if (huntCategoryIndex == 1)
          browseType = TargetType::STA;
        else
          browseType = TargetType::BLE;
    ...
    case PROFILE_VIEW: {
      ...
      if (mappedInput.wasReleased(MappedInputManager::Button::Down)) {
        ...
        const int visibleLines = (pageHeight - contentTop - metrics.buttonHintsHeight) / lh;
...
void HuntActivity::renderSelectCategory() const {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, "HUNT — Select Category");
  ...
  GUI.drawList(
      renderer, Rect{0, listTop, pageWidth, listHeight}, 3, huntCategoryIndex,
      [apCount, staCount, bleCount](int i) -> std::string {
        char buf[32];
        if (i == 0)
          snprintf(buf, sizeof(buf), "WiFi APs (%d)", apCount);
        else if (i == 1)
          snprintf(buf, sizeof(buf), "WiFi Clients (%d)", staCount);
        else
          snprintf(buf, sizeof(buf), "BLE Devices (%d)", bleCount);
...
void HuntActivity::renderSelectTarget() const {
  ...
  if (targetCount == 0) {
    ...
    GUI.drawList(
        renderer, Rect{0, headerBottom, pageWidth, listHeight}, targetCount, listIndex,
...
void HuntActivity::renderProfile() const {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, "HUNT — Profile");
  ...
  if (current->type == TargetType::AP) {
    ...
    snprintf(line, sizeof(line), "Channel: %d   Auth: %s", (int)current->channel, authTypeName(current->authType));
    renderer.drawText(UI_10_FONT_ID, x, y, line);
    y += lh;

    snprintf(line, sizeof(line), "PMF: %s   WPS: %s", current->pmf ? "Yes" : "No", current->wps ? "Yes" : "No");
    renderer.drawText(UI_10_FONT_ID, x, y, line);
    y += lh;
    ...
    if (current->hasHandshake || current->hasPmkid) {
      snprintf(line, sizeof(line), "Captured: %s%s", current->hasHandshake ? "EAPOL " : "",
  ...
  // --- Scroll hint ---
  if (profileScroll > 0) {
    renderer.drawText(SMALL_FONT_ID, x, metrics.topPadding + metrics.headerHeight + 2, "^ scroll up");
  }
...
void HuntActivity::renderCapabilities() const {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, "HUNT — Assessment");
  ...
  if (capCount == 0) {
    ...
    GUI.drawList(
        renderer, Rect{0, headerBottom, pageWidth, listHeight}, capCount, capIndex,
        [this](int i) -> std::string {
          char buf[64];
          snprintf(buf, sizeof(buf), "%s  [%s]", capabilities[i].name,
                   capabilities[i].available ? "available" : "limited");
          return std::string(buf);
        },
        [this](int i) -> std::string { return std::string(capabilities[i].reason); });
  }
...
void HuntActivity::renderClientList() const {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, "HUNT — AP Clients");
  ...
  if (clientCount2 == 0) {
    ...
    GUI.drawList(
        renderer, Rect{0, headerBottom, pageWidth, listHeight}, clientCount2, clientListIndex,
        [this](int i) -> std::string {
