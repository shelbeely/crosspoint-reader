...
void BarcodeActivity::launchKeyboard() {
  ...
  if (barcodeType == EAN13)
    title = "Enter 13 Digits";
  else if (barcodeType == CODE39)
    title = "Enter A-Z, 0-9, - . $/%+";

  startActivityForResult(std::make_unique<KeyboardEntryActivity>(renderer, mappedInput, title, inputText, 0),
...
void BarcodeActivity::render(RenderLock&&) {
  ...
  if (state == TYPE_SELECT) {
    GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, "Barcode Generator");
...
void BarcodeActivity::renderEan13() const {
  ...
  if (inputText.size() != 13) {
    renderer.drawCenteredText(UI_10_FONT_ID, (headerBottom + footerTop) / 2 - 20, "EAN-13 requires exactly 13 digits",
                              true);
    return;
  }
  for (char c : inputText) {
