...
void LootActivity::onExit() { Activity::onExit(); }
...
void LootActivity::loadFileDetail(int index) {
  ...
  if (state == ITEM_DETAIL && listCategory == CREDENTIALS) {
    ...
    if (index < credCount) {
      ...
      snprintf(detailBuf, sizeof(detailBuf), "SSID: %s\nUser: %s\nPass: %s\nTime: %s", cr.ssid[0] ? cr.ssid : "(empty)",
               cr.username[0] ? cr.username : "(empty)", cr.password[0] ? cr.password : "(empty)",
  ...
  if (listCategory == HANDSHAKES) {
    ...
    snprintf(detailBuf, sizeof(detailBuf), "File: %s\nSize: %u bytes\nStatus: %s", fileNames[index], (unsigned)fsize,
             status);
  } else if (listCategory == PCAP_FILES) {
    ...
    snprintf(detailBuf, sizeof(detailBuf), "File: %s\nSize: %u bytes\n~%d packets", fileNames[index], (unsigned)fsize,
             packetEst);
  } else {
    snprintf(detailBuf, sizeof(detailBuf), "File: %s\nSize: %u bytes", fileNames[index], (unsigned)fsize);
  }
}
...
void LootActivity::executeDetailAction() {
  ...
  if (detailAction == EXPORT && listCategory == CREDENTIALS) {
    ...
    if (fileIndex < credCount) {
      ...
      snprintf(detailBuf, sizeof(detailBuf), "WIFI:T:WPA;\nS:%s;\nP:%s;;", cr.ssid, cr.password);
    }
    requestUpdate();
    return;
  }
...
void LootActivity::renderOverview() const {
  ...
  GUI.drawList(
      renderer, Rect{0, contentTop, pageWidth, contentHeight}, CATEGORY_COUNT, overviewIndex,
  ...
  renderer.drawCenteredText(SMALL_FONT_ID, pageHeight - metrics.buttonHintsHeight - metrics.verticalSpacing - 18,
                            totalBuf);
...
void LootActivity::renderFileList(const char* title) const {
  ...
  if (fileCount == 0) {
    ...
    GUI.drawList(renderer, Rect{0, contentTop, pageWidth, contentHeight}, fileCount, fileIndex,
...
void LootActivity::renderCredentialList() const {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, "Credentials", subtitle);
  ...
  if (credCount == 0) {
    ...
    GUI.drawList(
        renderer, Rect{0, contentTop, pageWidth, contentHeight}, credCount, fileIndex,
        [this](int i) -> std::string { return creds[i].ssid[0] ? creds[i].ssid : "(no ssid)"; },
...
void LootActivity::renderDetail() const {
  ...
  renderer.drawCenteredText(SMALL_FONT_ID, pageHeight - metrics.buttonHintsHeight - metrics.verticalSpacing - 20,
                            actionBuf);
...
void LootActivity::render(RenderLock&&) {
  ...
  switch (state) {
    case OVERVIEW:
      ...
      break;
    case HANDSHAKES:
      renderFileList("Handshakes");
      break;
    case PCAP_FILES:
      renderFileList("PCAP Files");
      break;
    case BLE_DUMPS:
      renderFileList("BLE Dumps");
      break;
    case CREDENTIALS:
