...
static const uint8_t* findTag(const uint8_t* data, uint16_t len, int startOffset, uint8_t tag, uint8_t& outLen) {
...
bool FrameParser::isBeacon(const uint8_t* data, uint16_t len) { return len >= 24 && data[0] == 0x80; }

bool FrameParser::isProbeRequest(const uint8_t* data, uint16_t len) { return len >= 24 && data[0] == 0x40; }

bool FrameParser::isProbeResponse(const uint8_t* data, uint16_t len) { return len >= 24 && data[0] == 0x50; }

bool FrameParser::isDataFrame(const uint8_t* data, uint16_t len) { return len >= 24 && (data[0] & 0x0C) == 0x08; }

bool FrameParser::isAuthFrame(const uint8_t* data, uint16_t len) { return len >= 24 && data[0] == 0xB0; }
...
bool FrameParser::isEapolFrame(const uint8_t* data, uint16_t len) {
  ...
  return data[llcOffset] == 0xAA && data[llcOffset + 1] == 0xAA && data[llcOffset + 6] == 0x88 &&
...
bool FrameParser::extractSSID(const uint8_t* data, uint16_t len, char* ssid, int maxLen) {
...
uint8_t FrameParser::extractAuthType(const uint8_t* data, uint16_t len) {
  ...
  switch (akmType) {
    ...
    case 2:
      return 3;  // WPA2-PSK
    case 1:
      return 3;  // WPA2-Enterprise (same bucket)
    default:
      return 2;  // WPA
  }
}
...
void FrameParser::processPacket(const CapturedPacket& pkt) {
  ...
  if (isBeacon(data, len) || isProbeResponse(data, len)) {
    ...
    char probedSsid[33] = {};
    if (extractSSID(data, len, probedSsid, sizeof(probedSsid)) && probedSsid[0] != '\0') {
